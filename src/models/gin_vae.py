"""
GIN-VAE (Graph Isomorphism Network VAE):
  Uses GINConv in the encoder. GIN is theoretically proven to be as powerful
  as the Weisfeiler-Lehman graph test, making it a SOTA standard for 
...
class ResidualGINBlock(nn.Module):
    def __init__(self, in_dim: int, out_dim: int, dropout: float = 0.1):
        ...
        self.conv = GINConv(nn_callable, train_eps=True)
...
class GINVAE(GraphVAEBase):
    def __init__(self, config: TrainConfig):
        ...
        for i in range(n_layers):
            out_dim = hidden_dim
            encoder_layers.append(ResidualGINBlock(in_dim, out_dim, dropout=dropout))
            in_dim = out_dim
