"""
GAT-VAE (Graph Attention Network VAE):
  Uses Graph Attention Networks (GATConv) in the encoder.
  Attention allows the model to learn the importance of different neighbor nodes,
  making it highly suitable for complex molecules.
"""
...
class ResidualGATBlock(nn.Module):
    def __init__(self, in_dim: int, out_dim: int, heads: int = 4, dropout: float = 0.1):
        super().__init__()
        # GATConv concatenates heads, so out_channels must be out_dim // heads
        assert out_dim % heads == 0, "out_dim must be divisible by heads"
        head_dim = out_dim // heads
        
        self.conv = GATConv(in_dim, head_dim, heads=heads, dropout=dropout)
...
class GATVAE(GraphVAEBase):
    def __init__(self, config: TrainConfig):
        ...
        for i in range(n_layers):
            out_dim = hidden_dim
            encoder_layers.append(ResidualGATBlock(in_dim, out_dim, heads=4, dropout=dropout))
            in_dim = out_dim
